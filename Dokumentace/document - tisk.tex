...
% Nahradit druhou stranu zadáním
\AtBeginShipout{
	\ifnum\value{page}=2
	\AtBeginShipoutUpperLeftForeground{
		\raisebox{-\height}{\colorbox{white}{\includegraphics{zadani.pdf}}}
	}
	\fi
}
...
\begin{document}
	...
	%mono vs il2cpp, podporované platformy
	...
	% TilemapShadowCaster2DGenerator
	
	\subsection{Hledání cesty}
	
	Nepřátelská jednotka má v sobě implementovaný algoritmus A* pro nalezení cesty k jejímu aktuálnímu cíli. A* vychází z Dijkstrova algoritmu, ke kterému přidává heuristickou funkci. Hodnota heuristické funkce je u každého vrcholu grafu přičítána k jeho ceně a tím vzniká kombinovaná cena vrcholu, která určuje, jaký další dočasný vrchol bude vybrán při hledání cesty. Charakteristickým rysem tohoto algoritmu je, že byl navržen tak, aby využil znalosti cílových souřadnic a tím minimalizoval čas potřebný k nalezení řešení \cite{lit2}.
	
	Jako heuristická funkce byla použita euklidovská vzdálenost pomocí \textit{Vector2Int.Distance}. Vrcholy pomyslného grafu jsou jednotlivé dlaždice z tilemap. Pokud je pomocí \textit{Physics2D.OverlapArea} zjištěno, že se na nějaké dlaždici nachází objekt, je dlaždice (vrchol) prohlášena za neprůchozí a ignorována při hledání cesty.
	...
	V rámci projektu byla vytvořena demonstrativní videohra pro platformy Windows a Linux. Hra obsahuje kompletní příběhový mód a během testování se nevyskytly žádné technické problémy.
	
	Hráči je umožněno se s nepřáteli vypořádat buď s využitím boje na blízko, nebo je obejít pomocí plížení a boji se tak vyhnout. Mezi těmito dvěma přístupy může hráč libovolně přecházet a nepřátelská jednotka byla navržena tak, aby na oba přístupy byla schopna reagovat. Implementace jejího chování využívá stavového automatu a umožňuje snazší přidání nových stavů i nepřátel.
	
	Nejpropracovanější mechanikou je pravděpodobně systém plížení. Hru lze dohrát od začátku do konce bez zranění jediné nepřátelské jednotky. Pokud nepřítel někde zahlédne hráče, vydá se toto místo prozkoumat. Až když se hráč nachází v jeho zorném poli déle, přepne se do stavu pronásledování. Díky vizualizaci zorného pole nepřátel může hráč plánovat načasování a cestu svých kroků.
	...
	\section{Nedostatky} % Chybí audio, combat, accessibility features, DÉLKA HRY
	...
	%\section{Návrh na vylepšení} % přidat enemy, čtvercové monitory, mesh místo urp
